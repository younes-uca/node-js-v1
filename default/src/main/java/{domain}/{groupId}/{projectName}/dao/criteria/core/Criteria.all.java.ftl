package  ${config.domain}.${config.groupId}.${config.projectName}.${config.dao}.criteria.core;
...
import ${config.domain}.${config.groupId}.${config.projectName}.required.criteria.${dependency.msPackaging}.${dependency.name}Criteria;
        </#if>
    </#list>
</#if>
<#if !pojo.subEntity>
